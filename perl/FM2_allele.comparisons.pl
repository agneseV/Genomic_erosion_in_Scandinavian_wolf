#!usr/bin/perl
#This script is individually tailored for the second male founder analysis
#it reads in a file with unique alleles of the second male founder and determines if each particular allele 
#is present in the two temporal subgroups and bins unique alleles of the second male founder into 4 output files:
#TT (absent in both subgroups); FT (present in first, absent in the other), TF (vice versa), FF (present in both)
#usage: perl FM2_allele.comparisons.pl FM2_uniq_allele.table subgroup2.alele.table subgroup3.allele.table out.summary
...
open (BIN1, '>', "FM2-offsp.unique.allele.bin1.TT") or die "Can't open BIN1: $!\n";
