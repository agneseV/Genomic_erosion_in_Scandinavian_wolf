#!usr/bin/perl
#This script is individually tailored for analyzing the common alleles of the first and second male founders
#it reads in a file with common alleles of both male founders and determines if each particular allele 
#is present in the two temporal subgroups and bins common male alleles into 4 output files:
#TT (absent in both subgroups); FT (present in first, absent in the other), TF (vice versa), FF (present in both)
#usage: perl FM2_allele.comparisons.pl FM1_FM2_common_allele.table subgroup2.alele.table subgroup3.allele.table out.summary
...
#make output files to store coordinates of alleles in particular bin
open (BIN1, '>', "1983-1993_FM1_FM2_without_uniq_FM1_FM2_offsp.bin1.TT") or die "Can't open BIN1: $!\n";
...
while (<FH1>) {
        ...
        #loop through the subgroup3 and look for the line with corresponding chr and position of the allele
